          ...
          toast({
            ...
            description: (
              <div className="mt-2 text-sm">
                <p className="mb-2 font-medium">Mohon perbaiki kesalahan berikut:</p>
                <ul className="list-disc pl-4 space-y-1 text-xs">
                  {Object.entries(result.errors).map(([field, messages]) => (
                    <li key={field}>
                      <span className="capitalize font-semibold">{field}:</span> {messages[0]}
                    </li>
                  ))}
                </ul>
              </div>
  ...
  const handleDelete = async (userId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ variant: "destructive", title: "Gagal", description: result.error || "Gagal menghapus pengguna" });
      }
    } catch {
  ...
  const performToggle = async (userId: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ variant: "destructive", title: "Gagal", description: result.error || "Gagal mengubah status pengguna" });
      }
    } catch {
      toast({ variant: "destructive", title: "Error", description: "Terjadi kesalahan sistem" });
    } finally {
              ...
              <div className="space-y-1.5 sm:space-y-2">
                ...
                <Select name="role" defaultValue={editingUser?.role || "FIELD_STAFF"}>
                  ...
                  <SelectContent className="rounded-2xl border-border shadow-2xl">
                    <SelectItem value="FIELD_STAFF" className="font-bold py-3 cursor-pointer">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-muted-foreground font-medium">Akses operasional & laporan</span>
                        </div>
                      </div>
                    </SelectItem>
                    <SelectItem value="ADMIN" className="font-bold py-3 cursor-pointer">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-muted-foreground font-medium">Kendali penuh sistem</span>
                        </div>
                      </div>
                    </SelectItem>
                  </SelectContent>
            ...
            {users.map((user, index) => (
              ...
              >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        {user.isActive ? (
                          <div className="flex items-center gap-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                            <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Aktif Sekarang</span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-1.5">
                            <span className="w-1.5 h-1.5 rounded-full bg-red-400" />
                            <span className="text-[10px] font-black text-red-500 uppercase tracking-widest">Nonaktif</span>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <td className="px-8 py-6 bg-card group-hover:bg-muted/30 border-y border-border/50 transition-all duration-300">
                  <span className="text-sm font-bold text-foreground/80 tracking-tight">{user.email}</span>
                </td>
                <td className="px-8 py-6 bg-card group-hover:bg-muted/30 border-y border-border/50 transition-all duration-300 text-center">
                  <Badge 
                    ...
                    className={cn(
                      ...
                      user.role === "ADMIN" ? "bg-amber-500/10 text-amber-600 ring-1 ring-amber-500/20" : "bg-primary/5 text-primary ring-1 ring-primary/20"
                    )}
                  >
                    <div className="flex items-center gap-2">
                ...
                <td className="px-8 py-6 bg-card group-hover:bg-muted/30 border-y border-border/50 transition-all duration-300 text-center">
                  <div className="flex items-center justify-center gap-4">
                    ...
                    <span className={cn(
                      ...
                      user.isActive ? "text-emerald-600" : "text-muted-foreground"
                    )}>
                ...
                <td className="px-8 py-6 bg-card group-hover:bg-muted/30 border-y border-border/50 transition-all duration-300">
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-foreground tracking-tight">{new Date(user.createdAt).toLocaleDateString("id-ID", { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1">Registrasi Sistem</span>
                  </div>
                </td>
                <td className="px-8 py-6 bg-card group-hover:bg-muted/30 rounded-r-[2.5rem] border-y border-r border-border/50 transition-all duration-300 text-center">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-11 w-11 rounded-2xl bg-muted/30 hover:bg-card transition-all">
                        <MoreHorizontal className="h-5 w-5" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="rounded-[1.5rem] border-border shadow-2xl p-2 min-w-[200px] backdrop-blur-xl">
                      <DropdownMenuLabel className="px-4 py-2.5 text-[10px] font-black text-muted-foreground uppercase tracking-widest">Opsi Pengguna</DropdownMenuLabel>
                      <DropdownMenuItem className="rounded-xl py-3 px-4 font-bold text-xs cursor-pointer" onClick={() => { setEditingUser(user); setIsDialogOpen(true); }}>
                        <Pencil className="mr-3 h-4 w-4 text-primary" /> Edit Profil
                      </DropdownMenuItem>
                      <div className="h-px bg-border my-2 mx-1" />
                      <DropdownMenuItem className="rounded-xl py-3 px-4 font-bold text-xs cursor-pointer" onClick={() => handleToggleStatus(user)} disabled={togglingUserId === user.id}>
                        {user.isActive ? (
                          <div className="flex items-center text-orange-600">
                            <UserMinus className="mr-3 h-4 w-4" /> Nonaktifkan Akun
                          </div>
                        ) : (
                      ...
                      <DropdownMenuItem className="rounded-xl py-3 px-4 font-bold text-xs cursor-pointer text-red-600 focus:text-red-600 focus:bg-red-50" onClick={() => handleDelete(user.id)}>
