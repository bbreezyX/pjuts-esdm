...
import { Bell, CheckCircle2, AlertTriangle, XCircle, FileText, ArrowRight, Circle } from "lucide-react";
...
export function Notifications() {
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button 
                    ...
                    className={cn(
                        ...
                        unreadCount > 0 ? "bg-primary/5 hover:bg-primary/10" : "text-muted-foreground"
                    )}
                >
            ...
            <PopoverContent className="w-[320px] sm:w-[400px] p-0 rounded-3xl shadow-2xl border-border bg-card overflow-hidden" align="end" sideOffset={12}>
                <div className="flex items-center justify-between p-6 border-b border-border bg-muted/20">
                    <div>
                        <h4 className="font-bold text-sm text-foreground">Notifikasi</h4>
                        <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-widest mt-1">Updates terbaru</p>
                    </div>
                    {unreadCount > 0 && (
                        <Button
                            variant="ghost"
                ...
                <ScrollArea className="h-[400px]">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full p-12 text-center">
                            ...
                            <p className="text-sm font-bold text-foreground">Hening Sekali...</p>
                            <p className="text-xs text-muted-foreground mt-1">Belum ada notifikasi baru untuk Anda.</p>
                        </div>
                    ) : (
                                        ...
                                        <div className="flex-1 space-y-1 min-w-0">
                                            <div className="flex justify-between items-start gap-2">
                                                ...
                                                <span className="text-[10px] text-muted-foreground/60 whitespace-nowrap font-medium">
                                                    {formatDistanceToNow(new Date(notification.timestamp), { addSuffix: false, locale: id })}
                                                </span>
                                            </div>
