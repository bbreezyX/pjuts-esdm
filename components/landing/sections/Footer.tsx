...
export const Footer = () => {
  ...
  const socialItems = ["Instagram", "Twitter", "Facebook", "YouTube"];
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div>
            <div className="flex items-center gap-4 mb-10">
              ...
              <div className="flex flex-col -space-y-1">
                <span className="font-black tracking-tight text-foreground text-3xl">
            ...
            <div className="flex gap-6 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
              <p>
                © {new Date().getFullYear()} {t("nav.ministry")}
              </p>
              <a
                href="#"
            ...
            <div className="space-y-6">
              ...
              {navItems.map((item) => (
                <a
                  key={item}
                  href="#"
                  className="block text-sm font-bold hover:text-primary transition-colors tracking-tight"
                >
                  {item}
                </a>
              ))}
            ...
            <div className="space-y-6 col-span-2 sm:col-span-1">
              ...
              {socialItems.map((item) => (
                <a
                  key={item}
                  href="#"
                  className="flex items-center justify-between text-sm font-bold border-b border-border pb-3 hover:text-primary transition-all group tracking-tight"
                >
                  {item}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-10 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] pt-16 border-t border-border">
          <div className="flex flex-wrap justify-center gap-12">
            ...
            <span className="flex items-center gap-2">
              <div className="w-1 h-1 bg-primary rounded-full" />
              esdm@jambiprov.go.id
            </span>
          </div>
          <p className="text-center md:text-right">{t("footer.address")}</p>
        </div>
      </div>
    </footer>
  );
